...
function maskPhone(value) {
  if (!value) return "Đăng nhập để xem số điện thoại";
  const text = String(value);
  if (text.length <= 4) return "****";
  return `${text.slice(0, Math.min(4, text.length - 4))}${"*".repeat(Math.max(4, text.length - Math.min(4, text.length - 4)))}`;
}

function maskEmail(value) {
  if (!value) return "Đăng nhập để xem email";
  const [name = "", domain = ""] = String(value).split("@");
  const visible = name.slice(0, Math.min(3, name.length));
  return domain ? `${visible}${"*".repeat(Math.max(4, name.length - visible.length))}@${domain}` : `${visible}****`;
}
...
export default function ShopDetail() {
  ...
  const [buyerLoggedIn, setBuyerLoggedIn] = useState(hasBuyerSession);
    ...
    const loadData = async () => {
      try {
        ...
        setProducts(
          allProducts.filter(
            (product) =>
              String(product.vendorId || product.vendor_id) === String(id),
          ),
        );
  ...
  useEffect(() => {
    const syncBuyerSession = () => setBuyerLoggedIn(hasBuyerSession());
    window.addEventListener("buyer-auth-changed", syncBuyerSession);
    window.addEventListener("storage", syncBuyerSession);
    return () => {
      window.removeEventListener("buyer-auth-changed", syncBuyerSession);
      window.removeEventListener("storage", syncBuyerSession);
    };
  }, []);
  ...
  const shopCoverImage = useMemo(
    () =>
      getProductImage(
        activeProducts.find((product) => getProductImage(product)),
      ) ||
      vendor?.logoUrl ||
      "/logo192.png",
    [activeProducts, vendor?.logoUrl],
  );
  ...
  const totalProductPages = Math.max(
    1,
    Math.ceil(visibleProducts.length / SHOP_PRODUCTS_PAGE_SIZE),
  );
      ...
      <div className="premium-page">
        ...
        <div className="mx-auto max-w-7xl px-4 py-16 text-center space-y-4">
          ...
          <p className="text-lg font-extrabold text-slate-800">
            {error || "Không tìm thấy thông tin cửa hàng"}
          </p>
          <Link to="/" className="inline-flex text-sm font-bold text-[#ff4d2e]">
            Quay lại trang chủ
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="premium-page text-[#16202a]">
      ...
      <main className="premium-shell space-y-6 py-6">
        {/* Shop Header Banner Card */}
        <Card className="overflow-hidden rounded-[2rem] border border-slate-100 bg-white shadow-[0_30px_90px_-68px_rgba(15,23,42,0.75)]">
          <div className="relative h-48 overflow-hidden bg-slate-100 md:h-72">
            <img
              src={shopCoverImage}
              alt={vendor.shopName}
            ...
            <div className="absolute inset-x-0 bottom-0 h-28 bg-gradient-to-t from-white via-white/62 to-transparent" />
          </div>
          <CardContent className="relative p-5 pt-0 sm:p-6 sm:pt-0">
            <div className="-mt-14 mb-2 flex flex-col gap-5 rounded-[1.6rem] border border-white/82 bg-white/88 p-4 shadow-[0_26px_70px_-52px_rgba(15,23,42,0.72)] backdrop-blur-xl md:flex-row md:items-end md:p-5">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 flex-wrap">
                  <h1 className="text-2xl font-black text-slate-950 truncate">
                    {vendor.shopName}
                  </h1>
                ...
                <p className="mt-2 text-sm text-slate-500 font-semibold flex items-center gap-1">
                  ...
                  <span className="text-slate-850 font-bold">
                    {vendor.category || "Tất cả sản phẩm"}
                  </span>
                </p>
              </div>
              <div className="flex shrink-0 items-center gap-5 rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
              ...
              <Button
                type="button"
                variant="outline"
                className="shrink-0 rounded-2xl border-red-100 text-red-600 hover:-translate-y-0 hover:border-red-200 hover:bg-red-50 hover:text-red-700"
            ...
            <div className="grid gap-5 md:grid-cols-[1.5fr_1fr]">
              <div className="space-y-2">
                ...
                <p className="text-sm font-semibold leading-relaxed text-slate-650">
                  {vendor.description ||
                    "Cửa hàng chưa có mô tả giới thiệu cụ thể."}
                </p>
              </div>
              <div className="space-y-3 rounded-2xl border border-stone-100/70 bg-white/70 p-4 shadow-inner shadow-orange-50/40">
                ...
                <div className="space-y-2.5 text-sm font-semibold text-slate-600">
                  <div className="flex items-center gap-2">
                    ...
                    <span>{buyerLoggedIn ? (vendor.phone || "Chưa cập nhật số điện thoại") : maskPhone(vendor.phone)}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Mail className="h-4 w-4 text-slate-400 shrink-0" />
                    <span>{buyerLoggedIn ? (vendor.email || "Chưa cập nhật email") : maskEmail(vendor.email)}</span>
                  </div>
                  <div className="flex items-center gap-2">
        ...
        <section className="space-y-4 rounded-[2rem] border border-slate-100 bg-white p-4 shadow-[0_30px_90px_-70px_rgba(15,23,42,0.75)] sm:p-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm font-semibold text-slate-500">
                Đang hiển thị {pagedProducts.length}/{visibleProducts.length}{" "}
                sản phẩm.
              </p>
            </div>

            <div className="flex flex-col gap-3 rounded-2xl border border-slate-100 bg-slate-50/70 p-3 sm:flex-row sm:items-center">
              <span className="inline-flex items-center gap-2 text-xs font-extrabold uppercase tracking-wide text-slate-500">
                <SlidersHorizontal className="h-4 w-4 text-orange-500" />
                Bộ lọc
              </span>
              <label className="sr-only" htmlFor="shop-category-filter">
                Lọc theo danh mục
              </label>
              <select
                ...
                className="premium-select h-11 min-w-48 px-3 text-sm font-bold"
              >
              ...
              <label className="sr-only" htmlFor="shop-sort-filter">
                Sắp xếp
              </label>
              <select
                id="shop-sort-filter"
                ...
                className="premium-select h-11 min-w-52 px-3 text-sm font-bold"
              >
          ...
          {visibleProducts.length === 0 ? (
            <div className="premium-empty font-semibold text-slate-500">
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
              {pagedProducts.map((p, index) => {
                ...
                return <ProductCard key={p.id} index={index} {...mapped} />;
              })}
            </div>
          )}
          {visibleProducts.length > SHOP_PRODUCTS_PAGE_SIZE && (
            <div className="premium-panel flex flex-col items-center justify-between gap-4 rounded-2xl px-4 py-4 sm:flex-row">
              ...
              <div className="flex flex-wrap items-center justify-center gap-2">
                <button
                  ...
                  onClick={() =>
                    setProductPage((page) => Math.max(0, page - 1))
                  }
                ...
                <button
                  ...
                  onClick={() =>
                    setProductPage((page) =>
                      Math.min(totalProductPages - 1, page + 1),
                    )
                  }
                  className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-bold text-slate-600 transition hover:border-[#ff4d2e]/40 hover:text-[#ff4d2e] disabled:cursor-not-allowed disabled:opacity-45"
                >
