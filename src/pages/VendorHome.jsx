      ...
      <div className="min-h-screen lg:pl-64">
        <header className="vendor-topbar sticky top-0 z-30 flex h-[72px] items-center gap-3 px-4 sm:px-6 lg:px-8">
          ...
          <div className="ml-auto flex items-center gap-1 sm:gap-2">
            ...
            <button type="button" className="vendor-primary-button hidden sm:inline-flex" onClick={() => onToast({ title: 'Thêm sản phẩm', message: 'Đã mở flow tạo sản phẩm mới tại bước thông tin cơ bản.' })}><Plus className="h-4 w-4" />Thêm sản phẩm</button>
          </div>
        </header>
...
export default function VendorHome() {
  ...
  return <><VendorLayout activeSlug={section} onToast={setToast}><Page navigateTo={navigateTo} onToast={setToast} /></VendorLayout>{toast && <VendorToast toast={toast} onClose={() => setToast(null)} />}</>;
}
